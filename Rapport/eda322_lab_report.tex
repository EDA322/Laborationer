...
The register is implemented using a D-flip-flop and a multiplexer for the load enable signal. The register is implemented generically so that words of any size can be stored into the flip-flop. The memory unit consists of an arbitrary number of registers, also using generic implementation. It can be initiated with a data vector file, so simulaton becomes easier.

The bus is implemented using a 4-to-1-multiplexer rather than using tri-state buffers. In this way, no signals can overload, but there may be multiple select signals avtive. For this we have the error (ERR) signal, that activates when when at least two select signals are active. The select signal for the multiplexer and the error signal are implemented using dataflow, where minimizations of the boolean expressions have been made (see Fig.~\ref{fig:enc}).

\begin{figure}[h!]
	\centering
	\includegraphics[height=.5\textwidth]{Figurer/enc4b}
	\caption{The minimized gate logic for the select signals}
	\label{fig:enc}
\end{figure}

\begin{figure}[h!]
	\centering
	\includegraphics[height=.5\textwidth]{Figurer/data instruction}
	\caption{A simulation waveform when writing and reading data of a memory unit.}
	\label{fig:enc}
\end{figure}
